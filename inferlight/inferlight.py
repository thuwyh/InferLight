...
class InferLight:
    ...
    def register_http_endpoint(self, url:str, methods=['POST']):
        ...
        if self.http_predict_handler is None:
            raise NotImplementedError("register a http data dispatch function first!")
        self.http_frontend.add_route(self.http_predict_handler, url, methods=methods)


    def http_data_dispatch_fn(self, data_dispatch_fn):
        """注册http数据处理函数
